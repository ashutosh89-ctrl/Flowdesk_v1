...
export async function generateInvoicePDF(
  ...
  doc.setFontSize(24);
  doc.setTextColor(...primaryBlack);
  doc.text('INVOICE', pageWidth - margin, rightHeaderY + 6, { align: 'right' });
  rightHeaderY += 12;
  ...
  const items = (invoice.items || []).map((item) => [
    ...
    formatCurrency(item.amount || (item.quantity || 1) * (item.rate || 0), invoice.currency),
  ]);
  ...
  doc.setTextColor(...primaryBlack);
  doc.text(formatCurrency(invoice.subtotal, invoice.currency), totalsValueX, currentTotalY, { align: 'right' });
  currentTotalY += 5.5;
  ...
  if (invoice.discount && invoice.discount > 0) {
    ...
    doc.text(`-${formatCurrency(invoice.discount, invoice.currency)}`, totalsValueX, currentTotalY, { align: 'right' });
    currentTotalY += 5.5;
  }
  ...
  if (invoice.tax > 0 || (invoice.taxPercentage !== undefined && invoice.taxPercentage > 0)) {
    ...
    doc.setTextColor(...primaryBlack);
    doc.text(formatCurrency(invoice.tax, invoice.currency), totalsValueX, currentTotalY, { align: 'right' });
    currentTotalY += 5.5;
  }
  ...
  doc.text(formatCurrency(invoice.total, invoice.currency), totalsValueX, currentTotalY, { align: 'right' });
  currentTotalY += 6;
  ...
  if (paidAmt > 0) {
    ...
    doc.text(`-${formatCurrency(paidAmt, invoice.currency)}`, totalsValueX, currentTotalY, { align: 'right' });
    currentTotalY += 5.5;
    ...
    doc.text(formatCurrency(remaining, invoice.currency), totalsValueX, currentTotalY, { align: 'right' });
    currentTotalY += 6;
  }

  // ─── INSTRUCTIONS & NOTES (Left side of totals or below) ───
  let notesY = yPos;
  const notesWidth = contentWidth - totalsBoxWidth - 8;

  if (invoice.paymentInstructions && invoice.paymentInstructions.trim()) {
    doc.setFillColor(248, 250, 252);
    doc.roundedRect(margin, notesY, notesWidth, 18, 1.5, 1.5, 'F');
    doc.setDrawColor(...lightGray);
    doc.roundedRect(margin, notesY, notesWidth, 18, 1.5, 1.5, 'S');

    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7.5);
    doc.setTextColor(...primaryBlack);
    doc.text('PAYMENT INSTRUCTIONS', margin + 3.5, notesY + 4.5);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7.5);
    doc.setTextColor(...mediumGray);
    const piLines = doc.splitTextToSize(invoice.paymentInstructions, notesWidth - 7);
    doc.text(piLines.slice(0, 3), margin + 3.5, notesY + 8.5);
    notesY += 22;
  }
  ...
  if (invoice.notes && invoice.notes.trim()) {
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(...mediumGray);
    doc.text('NOTES & TERMS', margin, notesY + 4);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7.5);
    doc.setTextColor(...mediumGray);
    const noteLines = doc.splitTextToSize(invoice.notes, notesWidth);
    doc.text(noteLines.slice(0, 3), margin, notesY + 8);
    notesY += 14;
  }
  ...
  doc.setFontSize(7.5);
  doc.setTextColor(...primaryBlack);
  doc.text('Authorized Signature', margin, yPos + 4);

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.5);
  doc.setTextColor(...mediumGray);
  doc.text(studioName, margin, yPos + 8);
      ...
      <div class="invoice-sheet">
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style="text-align: right; width: 20%;">Amount (${getCurrencySymbol(invoice.currency)})</th>
            </tr>
          </thead>
          <tbody>
            ${lineItemsHtml}
        ...
        <div class="totals-section">
          <div class="instructions-box">
            ...
            ${
              invoice.notes
                ? `<div style="font-weight: 700; color: #0f172a; margin-top: 8px; margin-bottom: 4px;">Terms & Notes</div>
        ...
        <div class="signature-section">
          ${signatureUrl ? `<img src="${signatureUrl}" class="signature-img" alt="Authorized Signature" />` : ''}
          <div class="signature-line"></div>
          <div style="font-size: 11px; font-weight: 700; color: #0f172a;">Authorized Signature</div>
          <div style="font-size: 10px; color: #64748b;">${studioName}</div>
        </div>
