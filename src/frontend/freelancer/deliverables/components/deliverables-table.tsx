            ...
            deliverables.map((del) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="p-4">
                    {(() => {
                      const isOverdue = del.dueDate && new Date(del.dueDate) < new Date() &&
                        del.status !== 'approved' && del.status !== 'completed' && del.status !== 'archived';
                      return (
                        <div className={`flex items-center gap-1.5 text-xs ${isOverdue ? 'text-rose-400 font-semibold' : 'text-zinc-400'}`}>
                          <Calendar className={`w-3.5 h-3.5 ${isOverdue ? 'text-rose-400' : 'text-zinc-500'}`} />
                          <span>{del.dueDate}</span>
                          {isOverdue && <span className="px-1.5 py-0.5 rounded bg-rose-500/10 text-[10px] font-bold text-rose-400 border border-rose-500/20">OVERDUE</span>}
                        </div>
                      );
                    })()}
                  </td>
                  ...
                  <td className="p-4 text-right" onClick={(e) => e.stopPropagation()}>
                    <Dropdown
                      trigger={
                        <button className="p-2 rounded-xl text-zinc-400 hover:text-white hover:bg-white/10 transition-colors" aria-label="More actions">
