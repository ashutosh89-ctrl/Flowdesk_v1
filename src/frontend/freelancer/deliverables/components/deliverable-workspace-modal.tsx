...
export const DeliverableWorkspaceModal: React.FC<DeliverableWorkspaceModalProps> = ({
    ...
    const fetchUserName = async () => {
      try {
        ...
        if (user) {
          ...
          setCurrentUserName(profile?.full_name || user.email?.split('@')[0] || 'User');
        }
      } catch { /* ignore */ }
    };
    fetchUserName();
  ...
  const handleCreateVersion = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newVersionTag.trim() || isSubmittingVersion) return;
    setIsSubmittingVersion(true);

    // Get actual file info if a file was selected
    const actualFileName = newVersionFileName.trim() || 
      (selectedFileForUpload ? selectedFileForUpload.name : `${deliverable.title.toLowerCase().replace(/\s+/g, '-')}-${newVersionTag}.zip`);
    const actualFileSize = selectedFileForUpload ? 
      formatFileSize(selectedFileForUpload.size) : '4.2 MB';
  ...
  const handleAddFile = async (e: React.FormEvent) => {
    ...
    if (uploadQueue.length > 0) {
      ...
      for (let i = 0; i < uploadQueue.length; i++) {
        ...
        try {
          ...
          setUploadQueue((prev) =>
            prev.map((q, idx) => (idx === i ? { ...q, status: 'completed' } : q))
          );
          successCount++;
        } catch (err: any) {
          setUploadQueue((prev) =>
            prev.map((q, idx) => (idx === i ? { ...q, status: 'failed', error: err?.message || 'Upload failed' } : q))
          );
          failCount++;
        }
      }
      ...
      if (successCount > 0) {
        showToast(
          ...
          `Successfully attached ${successCount} file${successCount > 1 ? 's' : ''}${failCount > 0 ? ` (${failCount} failed)` : ''}.`,
    ...
    const actualFileName = newFileName.trim();
    const actualFileSize = selectedFileForUpload
      ? formatFileSize(selectedFileForUpload.size)
      : (newFileSize || '2.4 MB');
    const actualFileType = selectedFileForUpload
      ? getFileExtension(selectedFileForUpload.name)
      : (newFileType || 'pdf');
    ...
    try {
      ...
      showToast('File Attached', `Added "${actualFileName}" to deliverable package.`, 'success');
      setIsAddFileOpen(false);
      setNewFileName('');
      setSelectedFileForUpload(null);
      setUploadQueue([]);
      ...
      showToast('Upload Error', err?.message || 'Failed to attach file.', 'error');
    } finally {
      setIsSubmittingFile(false);
    }
  };
  ...
  const handleSubmitReview = async () => {
    if (!reviewAction || isSubmittingReview) return;
    ...
    <Modal isOpen={isOpen} onClose={onClose} size="xl">
      <div className="space-y-6 select-none -m-1">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-white/10">
          ...
          {/* Header Quick Actions - State-dependent */}
          <div className="flex flex-wrap items-center gap-2">
            {/* Draft / Revision: Show primary action to advance state */}
            {(deliverable.status === 'draft' || deliverable.status === 'revision_requested') && (
              <>
                <Button
                  variant="outline"
                  size="sm"
            ...
            {/* Submitted: Show review status + optional version */}
            {deliverable.status === 'submitted' && (
              <>
                <span className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-emerald-500/10 text-emerald-300 border border-emerald-500/20">
            ...
            {/* Approved: Show view/download */}
            {deliverable.status === 'approved' && (
              <span className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-white/10 text-white border border-white/15">
                ✓ Approved
              </span>
            )}
          </div>
        ...
        <div className="flex items-center gap-1 border-b border-white/10 overflow-x-auto pb-1 scrollbar-none">
          ...
          {[
            ...
            { key: 'files', label: `Files (${filesList.length})`, icon: <FileText className="w-4 h-4" /> },
            { key: 'versions', label: `Versions (${versionsList.length})`, icon: <GitCommit className="w-4 h-4" /> },
          ].map((tab) => (
            <button
              key={tab.key}
          ...
          <div className="relative shrink-0" data-more-menu>
            <button
              onClick={() => {
                const moreTabs = ['approval', 'comments', 'revisions', 'timeline', 'activity', 'notes'] as const;
                const isMoreActive = moreTabs.includes(activeTab as any);
                setActiveTab(isMoreActive ? activeTab : 'approval');
                setShowMoreMenu(!showMoreMenu);
              }}
              className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-semibold transition-all ${
                ['approval', 'comments', 'revisions', 'timeline', 'activity', 'notes'].includes(activeTab)
                  ? 'bg-white/10 text-white shadow-inner border border-white/15'
                  : 'text-zinc-400 hover:text-white hover:bg-white/[0.05]'
              }`}
            >
            ...
            {showMoreMenu && (
              <div className="absolute top-full left-0 mt-1 z-50 w-48 py-1 bg-zinc-900/95 border border-white/15 rounded-xl shadow-2xl backdrop-blur-xl">
                {[
                  { key: 'approval', label: 'Client Sign-Off', icon: <FileCheck className="w-3.5 h-3.5" /> },
                  { key: 'comments', label: `Comments (${commentsList.length})`, icon: <MessageSquare className="w-3.5 h-3.5" /> },
                  { key: 'revisions', label: `Revisions (${revisionsList.length})`, icon: <AlertTriangle className="w-3.5 h-3.5" /> },
                  { key: 'timeline', label: 'Timeline', icon: <Clock className="w-3.5 h-3.5" /> },
                  { key: 'activity', label: 'Activity Log', icon: <History className="w-3.5 h-3.5" /> },
                  { key: 'notes', label: 'Internal Notes', icon: <Lock className="w-3.5 h-3.5" /> },
                ].map((item) => (
                  <button
                    key={item.key}
                    onClick={() => { setActiveTab(item.key as any); setShowMoreMenu(false); }}
                  ...
                  >
                    ...
                    <div className="pt-3 border-t border-white/5 flex flex-wrap items-center justify-between text-xs text-zinc-400 gap-2">
                      <div className="flex items-center gap-4">
                        ...
                        <span>By: {ver.uploadedBy || 'You'}</span>
                      </div>
            ...
            <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/10 space-y-4">
              ...
              <form onSubmit={handleAddComment} className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="primary" size="sm" type="submit" isLoading={isSubmittingComment} leftIcon={<Send className="w-3.5 h-3.5" />}>
                    Post Comment
                  </Button>
                </div>
              </form>
        ...
        {/* 7+8. HISTORY TAB (Timeline + Activity combined) */}
        {(activeTab === 'timeline' || activeTab === 'activity') && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1 p-0.5 bg-zinc-900/60 border border-white/10 rounded-lg">
                <button
                  onClick={() => setActiveTab('timeline')}
                  className={`px-2.5 py-1 text-[10px] font-semibold rounded-md transition-colors ${
                    activeTab === 'timeline' ? 'bg-white/10 text-white' : 'text-zinc-400 hover:text-white'
                  }`}
                >Timeline</button>
                <button
                  onClick={() => setActiveTab('activity')}
                  className={`px-2.5 py-1 text-[10px] font-semibold rounded-md transition-colors ${
                    activeTab === 'activity' ? 'bg-white/10 text-white' : 'text-zinc-400 hover:text-white'
                  }`}
                >Activity</button>
              </div>
            </div>
            ...
            {activeTab === 'activity' && (
              <div className="space-y-2">
                {activityList.map((act) => (
                  <div key={act.id} className="p-3 rounded-xl bg-zinc-950 border border-white/5 flex items-center justify-between text-xs">
                    <div className="flex items-center gap-3">
      ...
      {isAddVersionOpen && (
        ...
        >
          <form onSubmit={handleCreateVersion} className="space-y-4">
            ...
            <div className="border-2 border-dashed border-white/20 rounded-xl p-6 text-center hover:border-blue-400/50 transition-colors">
              ...
              >
                ...
                <span className="text-xs text-zinc-400">
                  {selectedFileForUpload ? selectedFileForUpload.name : 'Click to select version package file'}
                </span>
                <span className="text-[10px] text-zinc-500">
                  ZIP, PDF, or asset package
                </span>
              </label>
            </div>
            ...
            {selectedFileForUpload && (
              <div className="flex items-center justify-between p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-blue-300 font-medium">{selectedFileForUpload.name}</span>
                  <span className="text-[10px] text-zinc-500">({formatFileSize(selectedFileForUpload.size)})</span>
                </div>
                <button
                  type="button"
                  onClick={() => {
            ...
            <div>
              <label className="block text-xs font-semibold text-zinc-300 uppercase tracking-wider mb-1">
                Asset Package File Name {selectedFileForUpload ? '(auto-filled)' : ''}
              </label>
              <input
                type="text"
            ...
            <div className="flex justify-end gap-2 pt-3">
              ...
              <Button variant="primary" type="submit" isLoading={isSubmittingVersion}>
                Publish Release
              </Button>
            </div>
          </form>
      ...
      {isAddFileOpen && (
        <Modal
          ...
          title={uploadQueue.length > 1 ? `Upload Multiple Files (${uploadQueue.length})` : 'Attach File to Deliverable'}
        >
          <form onSubmit={handleAddFile} className="space-y-4">
            {/* Real multi-file upload input */}
            <div className="border-2 border-dashed border-white/20 rounded-xl p-6 text-center hover:border-emerald-400/50 transition-colors">
                ...
                onChange={(e) => {
                  ...
                  if (files.length > 0) {
                    ...
                    if (files.length === 1) {
                      ...
                      setNewFileSize(formatFileSize(files[0].size));
                      setNewFileType(getFileExtension(files[0].name));
                    } else {
                      setSelectedFileForUpload(null);
                      setNewFileName('');
                    }
              ...
              />
              <label
                htmlFor="deliverable-add-file"
                className="cursor-pointer flex flex-col items-center gap-2"
              >
                <UploadCloud className="w-8 h-8 text-emerald-400" />
                <span className="text-xs text-zinc-300 font-medium">
                  {uploadQueue.length > 0
                    ? `${uploadQueue.length} file${uploadQueue.length > 1 ? 's' : ''} selected (click to change)`
                    : 'Click to select one or multiple files'}
                </span>
                <span className="text-[10px] text-zinc-500">
                  PDF, ZIP, DOC, MP4, PNG, JPG, FIG up to 50MB
                </span>
              </label>
            </div>

            {/* Multi-file Queue & Progress Display */}
            {uploadQueue.length > 0 && (
              <div className="space-y-3 p-4 rounded-xl bg-zinc-900/90 border border-white/10">
                {/* Overall Progress Header */}
                <div className="flex items-center justify-between text-xs">
                  <span className="font-semibold text-white">
                    {isSubmittingFile
                      ? `Uploading files... (${uploadQueue.filter((q) => q.status === 'completed').length} / ${uploadQueue.length} uploaded)`
                  ...
                  {uploadQueue.map((item, idx) => (
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0 pr-2">
                        ...
                        <span className="text-[10px] text-zinc-500 shrink-0 font-mono">({item.size})</span>
                      </div>

                      <div className="flex items-center gap-2 shrink-0">
                        {item.status === 'pending' && (
                          <span className="inline-flex items-center gap-1 text-[10px] font-mono px-2 py-0.5 rounded-full bg-zinc-800 text-zinc-400 border border-white/10">
                        ...
                        {item.status === 'uploading' && (
                          <span className="inline-flex items-center gap-1 text-[10px] font-mono px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20">
                            <Loader2 className="w-3 h-3 text-blue-400 animate-spin" /> Uploading...
                          </span>
                        )}
            ...
            {/* Manual entry fallback if no batch files selected */}
            {uploadQueue.length === 0 && (
              <>
                <div>
                  <label className="block text-xs font-semibold text-zinc-300 uppercase tracking-wider mb-1">
                  ...
                  <input
                    ...
                    placeholder="e.g. design-system-tokens.json"
                    className="w-full px-3.5 py-2 rounded-xl bg-zinc-900 border border-white/10 text-sm text-white focus:outline-none"
                    required
                  />
                </div>
            ...
            <div className="flex justify-end gap-2 pt-3">
              ...
              <Button variant="primary" type="submit" isLoading={isSubmittingFile}>
                {uploadQueue.length > 1 ? `Upload ${uploadQueue.length} Files` : 'Attach File'}
              </Button>
            </div>
          </form>
        </Modal>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {reviewAction === 'revision' && (
                <p className="text-[10px] text-amber-400 mt-1">Revision reason is required</p>
              )}
            </div>
            ...
            <div className="flex justify-end gap-2 pt-3 border-t border-white/10">
              ...
              <Button variant="primary" onClick={handleSubmitReview} isLoading={isSubmittingReview} disabled={reviewAction === 'revision' && !reviewNote.trim()}>
