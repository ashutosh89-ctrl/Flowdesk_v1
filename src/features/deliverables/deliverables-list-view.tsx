...
import { DeliverableService, ClientService, ProjectService } from '../../services';
import { FlowDeskStore } from '../../services/storage-store';
import { Deliverable, Client, Project, DeliverableStatus, ApprovalStatus } from '../../types';
...
export const DeliverablesListView: React.FC<DeliverablesListViewProps> = ({
  ...
  const [deliverables, setDeliverables] = useState<Deliverable[]>(() => FlowDeskStore.getDeliverables());
  ...
  const handleSubmitForReview = async (id: string) => {
    await DeliverableService.updateDeliverable(id, {
      status: 'submitted',
  ...
  const handleBulkAction = async (action: 'archive' | 'delete' | 'submit' | 'mark_ready') => {
    ...
    await DeliverableService.bulkUpdateDeliverables(selectedIds, action);
