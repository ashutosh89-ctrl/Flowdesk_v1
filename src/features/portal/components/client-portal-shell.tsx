...
interface ClientPortalShellProps {
  ...
  onRequestRevision: (deliverableId: string, reason: string, priority: string, comment: string) => Promise<void>;
  onUploadDocument: (docId: string, fileData: { fileName: string; size: string }) => Promise<void>;
...
export const ClientPortalShell: React.FC<ClientPortalShellProps> = ({
  ...
  const handleQuickActionSelect = (type: 'upload' | 'approve' | 'comment' | 'pay' | 'download_zip') => {
    if (type === 'approve') {
      ...
      showToast('Workspace ZIP package generated! Mock download initiated.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white font-sans antialiased flex flex-col lg:flex-row selection:bg-white selection:text-zinc-950">
            ...
            <>
              ...
              {activeTab === 'overview' && (
                <OverviewCards
                  ...
                  onPayClick={async (inv) => {
                    await onPayInvoice(inv.id);
                    showToast(`Statement #${inv.invoiceNumber} paid successfully!`);
                  }}
                />
              )}
              ...
              {activeTab === 'deliverables' && (
                <DeliverablesPanel
                  ...
                  onDownloadClick={(deliv) => showToast(`Downloading ${deliv.title} (${deliv.version || 'v1.0'})...`)}
                />
              )}
              ...
              {activeTab === 'documents' && (
                <DocumentsPanel
                  ...
                  onDownloadClick={(doc) => showToast(`Downloading document: ${doc.title}...`)}
                  onFulfillRequestClick={async (req) => {
                    await onUploadDocument(req.id, {
                      fileName: `${req.title.toLowerCase().replace(/\s+/g, '-')}-fulfilled.pdf`,
                      size: '1.8 MB',
                    });
              ...
              {activeTab === 'invoices' && (
                <InvoicesPanel
                  ...
                  onPayClick={async (inv) => {
                    await onPayInvoice(inv.id);
                    showToast(`Statement #${inv.invoiceNumber} paid via Razorpay integration!`);
                  }}
                  onDownloadClick={(inv) => showToast(`Downloading PDF for #${inv.invoiceNumber}...`)}
                />
              )}
      ...
      <RevisionModal
        ...
        onSubmitRevision={async (delivId, reason, priority, comment) => {
          await onRequestRevision(delivId, reason, priority, comment);
          setSelectedRevisionDeliverable(null);
          showToast('Revision request submitted to Alex Rivera.');
        }}
      />
