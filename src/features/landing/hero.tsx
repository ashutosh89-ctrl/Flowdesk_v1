...
import { ArrowRight, CheckCircle2, Shield, Sparkles, Terminal } from 'lucide-react';
...
export const Hero: React.FC<HeroProps> = ({ onLaunchApp, onOpenAuth }) => {
  ...
  return (
    <section
      ...
      className="relative pt-32 pb-24 px-6 lg:px-12 overflow-hidden flex flex-col items-center text-center select-none"
    >
      {/* Monochrome Canvas Fluid Motion Layer */}
      <MonochromeFluidCanvas />

      {/* Subtle Premium Badge (No dev phase numbers) */}
      <motion.div
        initial={{ opacity: 0, y: -12 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
        className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/[0.05] border border-white/15 backdrop-blur-2xl text-xs font-medium text-zinc-300 mb-8 shadow-[0_8px_32px_rgba(0,0,0,0.5),inset_0_1px_0_0_rgba(255,255,255,0.2)]"
      >
        <span className="w-2 h-2 rounded-full bg-white shadow-[0_0_10px_rgba(255,255,255,1)] animate-pulse" />
      ...
      <motion.p
        ...
        className="text-base sm:text-lg text-zinc-400 max-w-2xl mt-6 leading-relaxed font-normal"
      >
      ...
      >
        <Button
          ...
          size="lg"
          onClick={onLaunchApp}
          className="shadow-[0_0_24px_rgba(255,255,255,0.15)] hover:shadow-[0_0_35px_rgba(255,255,255,0.3)] transition-all duration-300"
      ...
      {/* Parallax Interactive Crystal Glass Dashboard Showcase Preview */}
      <motion.div
      ...
      >
        ...
        <div className="rounded-2xl border border-white/20 bg-zinc-950/85 backdrop-blur-3xl shadow-[0_30px_90px_-20px_rgba(0,0,0,0.95),inset_0_1px_0_0_rgba(255,255,255,0.25)] p-5 sm:p-7 overflow-hidden group hover:border-white/30 transition-colors duration-500">
          {/* Mock Window Top Bar */}
          <div className="flex items-center justify-between pb-4 mb-5 border-b border-white/10 text-xs text-zinc-400">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
            {/* Stat Card 1 */}
            <div className="p-4 rounded-xl bg-white/[0.04] border border-white/10 backdrop-blur-md hover:border-white/25 hover:bg-white/[0.07] transition-all duration-300">
